...
router.get('/futopcoes', async (req, res) => {
  ...
  try {
    ...
    // Extrai o JSON do script ld+json
    let imagem = null;
    ...
    if (jsonldRaw) {
      try {
        ...
        if (parsed.image && parsed.image.url) {
          imagem = parsed.image.url;
        }
      } catch (err) {
        console.warn('⚠️ Erro ao parsear JSON-LD:', err.message);
      }
    }
...
router.get('/likes', async (req, res) => {
  ...
  console.log(`📩 [LIKE]: Enviando likes para ID = ${req.query.id}`);

  const getUserInfo = async () => {
    try {
      const { data } = await axios.get(`https://system.ffgarena.cloud/api/info_avatar?uid=${req.query.id}&region=${req.query.region || 'br'}`);
      return {
        likes: data.basicInfo?.liked || 0,
        nick: data.basicInfo?.nickname || 'Desconhecido',
        nivel: data.basicInfo?.level || 0,
        avatar: data.avatars || '',
      };
    } catch (e) {
      return null;
  ...
  if (!infoAntes) return res.json({ status: false, resultado: 'Erro ao consultar antes.' });

  try {
    // 👉 Colocar eventHandler ANTES de mandar a mensagem
    const handleResponse = new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        client.removeEventHandler(eventHandler);
        reject({ status: false, resultado: '⏰ Tempo esgotado.' });
      }, 30000);
      ...
      const eventHandler = async (event) => {
        ...
        console.log('📥 Recebido:', msg.message);
        ...
        if (!infoDepois) {
          return resolve({ status: false, resultado: 'Erro ao consultar depois.' });
        }

        return resolve({
          status: true,
          resultado: {
            likesAntes: infoAntes.likes,
            likesDepois: infoDepois.likes,
            likesGanhos: infoDepois.likes - infoAntes.likes,
            nick: infoDepois.nick,
            nivel: infoDepois.nivel,
    ...
    // 👇 Só manda a mensagem AGORA, depois do handler estar pronto
    ...
    if (!res.headersSent) {
      return res.json({ status: false, resultado: 'Erro geral.' });
    }
  }
});

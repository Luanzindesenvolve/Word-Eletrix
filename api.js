...
//play
// Defina suas rotas usando router
...
async function ytPlayMp3(query) {
    try {
        ...
        const video = searchResult.videos[0]; // Pega o primeiro resultado
        ...
        return {
            ...
            link: audioFormat.url // URL para baixar o áudio
        };
    } catch (error) {
        console.error(`Erro ao buscar MP3: ${error.message}`);
        throw error;
    }
}
...
async function ytPlayMp4(query) {
    try {
        ...
        const videoInfo = await yt.getInfo(video.url);
        const videoFormat = videoInfo.formats.find(format => format.container === 'mp4' && format.hasAudio && format.hasVideo);
        ...
        return {
            ...
            publicado: video.ago,
            visualizações: video.views,
            link: videoFormat.url // URL para baixar o vídeo
        };
    } catch (error) {
        console.error(`Erro ao buscar MP4: ${error.message}`);
        throw error;
    }
}

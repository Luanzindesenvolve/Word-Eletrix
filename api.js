...
// Rota para tratar o canal com a URL real
router.get('/canal/:canalUrl', async (req, res) => {
  const canalUrl = decodeURIComponent(req.params.canalUrl);  // Decodifica o link real que foi passado na URL
  
  try {
    // Fazendo a requisição para a API do iframe com a URL real
    const response = await axios.get(`https://world-ecletix.onrender.com/api/iframe?canal=${encodeURIComponent(canalUrl)}`);
    
    // Extrai o link do iframe da resposta da API
    const iframeUrl = response.data.iframe;

    // Envia a página com o iframe
    res.send(`
      <!DOCTYPE html>
      <html lang="pt-BR">
        <head>
    ...
    res.status(500).send('Erro ao carregar o canal.');
  }
});

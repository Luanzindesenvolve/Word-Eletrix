...
router.get('/audio', async (req, res) => {
  ...
  try {
    ...
    const downloadLink = response.data?.resultado?.result?.downloads?.audio?.any4k;
    if (!downloadLink) {
      return res.status(404).json({ error: 'Link de download de áudio não encontrado' });
    }
...
router.get('/video', async (req, res) => {
  ...
  try {
    ...
    const downloadLink = response.data?.resultado?.result?.downloads?.audio?.any4k;
    if (!downloadLink) {
      return res.status(404).json({ error: 'Link de download de vídeo não encontrado' });
    }

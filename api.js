  ...
  try {
    ...
    const handleResponse = new Promise((resolve, reject) => {
      const eventHandler = async (event) => {
        ...
        // Verifica se é a mensagem final do SPAMVIP
        if (msg.includes('✅ SPAMVIP FINALIZADO!')) {
          client.removeEventHandler(eventHandler);

          // Extrai dados importantes
          const info = {
            nome: (msg.match(/Nome:\s*(.+)/) || [])[1]?.trim(),
            regiao: (msg.match(/Região:\s*(.+)/) || [])[1]?.trim(),
            level: (msg.match(/Level:\s*(\d+)/) || [])[1]?.trim(),
            likes: (msg.match(/Likes:\s*(\d+)/) || [])[1]?.trim(),
            duracao: (msg.match(/Duração:\s*(.+)/) || [])[1]?.trim(),
            ciclos: (msg.match(/Ciclos:\s*(\d+)/) || [])[1]?.trim(),
            contas: (msg.match(/Contas:\s*(\d+)/) || [])[1]?.trim(),
            convites: (msg.match(/Total Convites:\s*(\d+)/) || [])[1]?.trim(),
          };
...
router.get('/spamsala', async (req, res) => {
  try {
    ...
    console.log(`[SPAM SALA]: ID = ${id}`);
    await client.sendMessage(grupoChatId, { message: `/spamsala ${id}` });
    ...
    const handleResponse = new Promise((resolve, reject) => {
      const eventHandler = async (event) => {
        ...
        // Espera mensagem final do JOINROOM
        if (msg.includes('✅ JOINROOM FINALIZADO!')) {
          client.removeEventHandler(eventHandler);

          // Extrai dados da sala
          const info = {
            sala: (msg.match(/Sala:\s*(\d+)/) || [])[1]?.trim(),
            duracao: (msg.match(/Duração:\s*(.+)/) || [])[1]?.trim(),
            ciclos: (msg.match(/Ciclos:\s*(\d+)/) || [])[1]?.trim(),
            contas: (msg.match(/Contas:\s*(\d+)/) || [])[1]?.trim(),
            pacotes: (msg.match(/Total Pacotes:\s*(\d+)/) || [])[1]?.trim(),
            canal: (msg.match(/Canal:\s*(.+)/) || [])[1]?.trim(),
            grupo: (msg.match(/Grupo:\s*(.+)/) || [])[1]?.trim(),
            criador: (msg.match(/Criador:\s*(.+)/) || [])[1]?.trim(),
          };

...
router.get('/boneco', async (req, res) => {
  try {
    ...
    const partes = [
      { nome: 'chapeu', url: req.query.chapeu, x: 180, y: 20, width: 150, height: 150 },
      { nome: 'mascara', url: req.query.mascara, x: 180, y: 180, width: 150, height: 120 },
      { nome: 'maquiagem', url: req.query.maquiagem, x: 180, y: 270, width: 150, height: 120 },
      { nome: 'blusa', url: req.query.blusa, x: 160, y: 320, width: 190, height: 220 },
      { nome: 'calca', url: req.query.calca, x: 170, y: 550, width: 170, height: 200 },
      { nome: 'sapato', url: req.query.sapato, x: 180, y: 770, width: 160, height: 100 },
    ];
    ...
    for (const parte of partes) {
      if (parte.url && parte.url.trim() !== '') {
        try {
          ...
          ctx.drawImage(img, parte.x, parte.y, parte.width, parte.height);
        } catch (err) {
          console.error(`Erro ao carregar imagem da parte ${parte.nome}:`, err.message);
        }
      }
    }

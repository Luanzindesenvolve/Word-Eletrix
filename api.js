...
router.get('/likes', async (req, res) => {
  ...
  const getUserInfo = async () => {
    try {
      ...
      console.error('❌ Erro ao buscar dados da API FF:', error.message);
      return null;
    }
  };
    ...
    const handleResponse = new Promise((resolve, reject) => {
      ...
      const eventHandler = async (event) => {
        try {
          ...
          // Só remove handler após receber qualquer resposta
          client.removeEventHandler(eventHandler);
          clearTimeout(timeout);

          // Consulta API novamente após a resposta do grupo (sem delay fixo)
...
• Avatar: ${infoDepois.avatar || 'Não encontrado'}

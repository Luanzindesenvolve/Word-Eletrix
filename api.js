...
router.get('/rgb', async (req, res) => {
    ...
    for (let i = 0; i < frames; i++) {
        ...
        // Gera uma cor RGB aleatória para o texto a cada frame
        const r = Math.floor(Math.random() * 256);
        const g = Math.floor(Math.random() * 256);
        const b = Math.floor(Math.random() * 256);
        ctx.fillStyle = `rgb(${r}, ${g}, ${b})`;

        // **AUMENTEI O TAMANHO DA FONTE PARA 80PX**
        ctx.font = 'bold 80px "Arial Narrow"';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';

        const totalHeight = wrapText(ctx, texto, largura / 2, altura / 2, maxWidth, 90);
        const verticalCenter = (altura - totalHeight) / 2 + (totalHeight / 2);

        wrapText(ctx, texto, largura / 2, verticalCenter, maxWidth, 90);

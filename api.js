...
router.get('/likes', async (req, res) => {
  ...
  const grupoChatId = -1002208588695; // substitua pelo ID real do grupo
    ...
    const handleResponse = new Promise((resolve, reject) => {
      ...
      const eventHandler = async (event) => {
        try {
          ...
          if (!msg.message.includes(id)) return;

          client.removeEventHandler(eventHandler);
          clearTimeout(timeout);

          // Espera 2 segundos antes da consulta final
          await new Promise(resolve => setTimeout(resolve, 2000));

          // Likes depois
          const infoDepois = await getUserInfo();
          if (!infoDepois) {
          ...
          return resolve({
            status: true,
            resultado: {
              likesAntes: infoAntes.liked || 0,
              likesDepois: infoDepois.liked || 0,
              nick: infoDepois.nickname || 'Desconhecido',
              nivel: infoDepois.level || 0,
...
router.get('/likes', async (req, res) => {
  ...
  console.log(`📩 [LIKE]: Solicitado envio de likes para ID = ${req.query.id}`);

  const getUserInfo = async () => {
    try {
      const { data } = await axios.get(`https://system.ffgarena.cloud/api/info_avatar?uid=${req.query.id}&region=${req.query.region || 'br'}`);
      return data.basicInfo || {};
    } catch (error) {
      console.error('❌ Erro ao buscar dados da API:', error.message);
      return null;
  ...
  try {
    await client.sendMessage(-1002208588695, { message: `/likes ${req.query.id}` });
    console.log(`✅ Mensagem enviada para o grupo: /likes ${req.query.id}`);
    ...
    const handleResponse = new Promise((resolve, reject) => {
      ...
      const eventHandler = async (event) => {
        try {
          const msg = event.message;
          if (!msg || !msg.message) return;
          if (!msg.message.includes(req.query.id)) return;

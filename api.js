...
router.get('/likesff', async (req, res) => {
    ...
    // Usa 100 como padrão e limita a 100 no máximo
    let quantidade = parseInt(quantity) || 100;
    if (quantidade > 100) quantidade = 100;

    try {
        const apiUrl = `https://kryptorfree.squareweb.app/like?uid=${encodeURIComponent(id)}&quantity=${quantidade}&key=bykryptor`;
        const response = await axios.get(apiUrl);
        const data = response.data;

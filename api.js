...
router.get("/linkmp3", async (req, res) => {
  ...
  if (!videoUrl) return res.status(400).json({ error: "Informe ?url=" });
  try {
    const videoId = new URL(videoUrl).searchParams.get("v");
    console.log("[playlink] Video ID:", videoId);

    const { data: html } = await axios.get(`https://any4k.com/pt/download/youtube/${videoId}`);
    console.log("[playlink] HTML baixado, tamanho:", html.length);

    const match = html.match(/window\.__NUXT__=(.*)<\/script>/);
    if (!match) return res.status(500).json({ error: "Não foi possível extrair dados" });

    const sandbox = { window: {} };
    vm.createContext(sandbox);
    vm.runInContext(`window.__NUXT__=${match[1]}`, sandbox);
    const nuxt = sandbox.window.__NUXT__;
    const data = nuxt.data[Object.keys(nuxt.data)[0]].data;
    const audios = data.raw_audio || [];
    console.log("[playlink] Áudios encontrados:", audios.map(a => a.ext));

    if (!audios.length) return res.status(500).json({ error: "Nenhum áudio encontrado" });

    const audio = audios.find(a => a.ext === "m4a") || audios[0];
    const deviceId = uuidv4().replace(/-/g, "");
    console.log("[playlink] Device ID:", deviceId);
    ...
    console.log("[playlink] Resposta da API Any4K:", downloadData);

    if (downloadData.err_code !== 0) return res.status(500).json({ error: "Falha ao requisitar link" });

    const fileUrl = `https://api.any4k.com/v1/file/o?i=${downloadData.id}`;
    console.log("[playlink] Redirecionando para:", fileUrl);
    return res.redirect(fileUrl);

  } catch (err) {
    console.error("[playlink] Erro interno:", err.message);
...
router.get("/linkmp4", async (req, res) => {
  ...
  try {
    const videoId = new URL(videoUrl).searchParams.get("v");
    console.log("[clipelink] Video ID:", videoId);

    const { data: html } = await axios.get(`https://any4k.com/pt/download/youtube/${videoId}`);
    console.log("[clipelink] HTML baixado, tamanho:", html.length);
    ...
    const nuxt = sandbox.window.__NUXT__;
    const data = nuxt.data[Object.keys(nuxt.data)[0]].data;
    const rawVideos = data.raw_video || [];
    const rawAudios = data.raw_audio || [];

    console.log("[clipelink] Raw vídeos:", rawVideos.map(v => v.ext));
    console.log("[clipelink] Raw áudios:", rawAudios.map(a => a.ext));

    const mp4Videos = rawVideos.filter(v => v.vcodec === "avc1");
    if (!mp4Videos.length) return res.status(500).json({ error: "Nenhum vídeo MP4 encontrado" });

    const video = mp4Videos[mp4Videos.length - 1];
    const audio = rawAudios.find(a => a.ext === "m4a") || rawAudios[0];
    const formatId = `${video.id}+${audio.id}`;
    console.log("[clipelink] Format combinado:", formatId);

    const deviceId = uuidv4().replace(/-/g, "");
    console.log("[clipelink] Device ID:", deviceId);
    ...
    console.log("[clipelink] Resposta da API Any4K:", downloadData);

    if (downloadData.err_code !== 0) return res.status(500).json({ error: "Falha ao requisitar link" });

    const fileUrl = `https://api.any4k.com/v1/file/o?i=${downloadData.id}`;
    console.log("[clipelink] Redirecionando para:", fileUrl);
    return res.redirect(fileUrl);

  } catch (err) {
    console.error("[clipelink] Erro interno:", err.message);
    return res.status(500).json({ error: "Erro interno", details: err.message });
  }
});
...
router.get("/musica", async (req, res) => {
  ...
  try {
    ...
    return res.redirect(`/playlink?url=${video.url}`);
...
router.get("/clipe", async (req, res) => {
  ...
  try {
    ...
    return res.redirect(`/clipelink?url=${video.url}`);

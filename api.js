...
router.get('/linkmp3', async (req, res) => {
    ...
    try {
        const response = await axios.get(`https://api.nexfuture.com.br/api/downloads/youtube/playaudio/v2?query=${encodeURIComponent(youtubeUrl)}`);
...
router.get('/linkmp4', async (req, res) => {
  const { youtubeUrl } = req.query.url;
  if (!youtubeUrl) return res.status(400).send('Parâmetro url é obrigatório');

  try {
    const response = await axios.get(`https://api.nexfuture.com.br/api/downloads/youtube/playvideo/v2?query=${encodeURIComponent(youtubeUrl)}`);

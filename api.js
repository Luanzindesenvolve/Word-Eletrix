...
router.get('/rgb', async (req, res) => {
    ...
    const largura = 512; // WhatsApp recomenda 512x512 para stickers
    const altura = 512;
    const frames = 10;  // Número de frames no GIF
    const delay = 100;  // Tempo entre frames em ms
    const maxWidth = largura * 0.8; // Largura máxima do texto

    // Inicializa o encoder de GIF
    const encoder = new GIFEncoder(largura, altura);
    encoder.start();
    encoder.setRepeat(0);
    encoder.setDelay(delay);
    encoder.setTransparent(0x000000); // Define a transparência
    ...
    for (let i = 0; i < frames; i++) {
        ctx.clearRect(0, 0, largura, altura); // Garante fundo transparente

        // Define uma cor aleatória para o texto
        const r = Math.floor(Math.random() * 256);
        const g = Math.floor(Math.random() * 256);
        const b = Math.floor(Math.random() * 256);
        ctx.fillStyle = `rgb(${r}, ${g}, ${b})`;

        ctx.font = 'bold 60px "Arial Narrow"';
    ...
    // Converte o GIF para WebP animado antes de enviar

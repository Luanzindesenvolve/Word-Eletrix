...
router.get("/audio-alice", (req, res) =>
  generateAudio(res, "nH8xrCazF1GzWmlk6P4l", req.query.text)
);
...
router.get('/fut/:slug', async (req, res) => {
  ...
  try {
    const { data } = await axios.get('https://world-ecletix.onrender.com/api/futopcoes?url=' + encodeURIComponent('https://multicanais.casa/' + req.params.slug));
...
router.get('/likes', async (req, res) => {
  ...
  console.log(`📩 [LIKE]: Solicitado envio de likes para ID = ${id}`);

  const getUserInfoAntes = async () => {
    try {
      const { data } = await axios.get(`https://system.ffgarena.cloud/api/info_avatar?uid=${id}&region=${region}`);
      const info = data.basicInfo || {};
      return {
        nickname: info.nickname || 'Desconhecido',
        level: info.level || 0,
        exp: info.exp || 0,
        region: info.region || region.toUpperCase(),
        likes: info.liked || 0,
        avatar: data.avatars || null,
      };
    } catch (error) {
      console.error('❌ Erro ao buscar dados da API FF (antes):', error.message);
      return null;
    }
  };

  const getUserInfoDepois = async () => {
    try {
      const { data } = await axios.get(`https://kryptorinfo.squareweb.app/api/player_info?uid=${id}`);
      const info = data.basicInfo || {};
      return {
        nickname: info.nickname || 'Desconhecido',
        level: info.level || 0,
        ...
        region: info.region || region.toUpperCase(),
        likes: info.liked || 0,
        avatar: data.avatars || 'Não encontrado',
      };
    } catch (error) {
      console.error('❌ Erro ao buscar dados após envio (nova API):', error.message);
      return null;
    }
  };

  const infoAntes = await getUserInfoAntes();
  if (!infoAntes) {
  ...
  try {
    ...
    console.log(`✅ Mensagem enviada para o grupo ${grupoChatId}: /likes ${id}`);

    const handleResponse = new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        client.removeEventHandler(eventHandler);
        reject({ status: false, resultado: '⏰ Tempo de resposta esgotado.' });
      }, 30000);
      ...
      const eventHandler = async (event) => {
        try {
          ...
          const resposta = msg.message;
          console.log('📥 Mensagem recebida do grupo:', resposta);

          client.removeEventHandler(eventHandler);
          clearTimeout(timeout);

          // Aguarda 3 segundos antes de consultar os likes depois
          ...
          while (tentativa < maxTentativas) {
            tentativa++;
            console.log(`🔁 Tentativa ${tentativa}: consultando likes depois...`);
            infoDepois = await getUserInfoDepois();

            if (!infoDepois) {
              return resolve({ status: false, resultado: 'Erro ao consultar informações após envio.' });
            }

            if (infoDepois.likes > infoAntes.likes) {
              console.log(`✅ Likes aumentaram: ${infoAntes.likes} → ${infoDepois.likes}`);
              break;
            }

            // Se ainda não aumentou, aguarda 2 segundos antes de tentar de novo
          ...
          if (infoDepois.likes <= infoAntes.likes) {
            return resolve({
              status: false,
              resultado: `⚠️ Likes não aumentaram após ${maxTentativas} tentativas.\n\nLikes Antes: ${infoAntes.likes}\nLikes Agora: ${infoDepois.likes}`,
            });
          }

          const resultadoFinal = `LIKES ENVIADOS COM SUCESSO! ✨

• Nick: ${infoDepois.nickname}
• UID: ${id}
• Nível: ${infoDepois.level} (Exp: ${infoDepois.exp})
• Região: ${infoDepois.region}
...
🗨️ Resposta: ${resposta}`;
          ...
          resolve({ status: true, resultado: resultadoFinal });

        } catch (err) {
          console.error('❌ Erro ao processar resposta:', err);
          client.removeEventHandler(eventHandler);
          clearTimeout(timeout);
          reject({ status: false, resultado: 'Erro ao processar resposta do grupo.' });
        }
      };

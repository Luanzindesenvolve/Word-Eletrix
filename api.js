...
router.get('/jogosdodia', async (req, res) => {
  try {
    const { data } = await axios.get('https://multicanais.gg/');
    const $ = cheerio.load(data);
    const jogos = [];
...
router.get('/ufc', async (req, res) => {
  try {
    const siteUrl = 'https://multicanais.gg/assistir/ufc-ao-vivo/';
...
router.get('/bbb25', (req, res) => {
  const resultado = [
    "https://multicanais.gg/assistir-bbb-ao-vivo-online-24-horas/",
...
router.get('/basquete', async (req, res) => {
  try {
    const siteUrl = 'https://multicanais.gg/assistir/basquete-ao-vivo/';
    const { data } = await axios.get(siteUrl);
...
router.get('/nfl', async (req, res) => {
  try {
    const siteUrl = 'https://multicanais.gg/assistir/nfl-ao-vivo/';
    const { data } = await axios.get(siteUrl);
 ...
 router.get('/ucl', async (req, res) => {
  try {
    const siteUrl = 'https://multicanais.gg/assistir/champions-league-ao-vivo/';
    const { data } = await axios.get(siteUrl);
...
router.get('/brasileirao', async (req, res) => {
  try {
    const siteUrl = 'https://multicanais.gg/assistir/brasileiro-ao-vivo/';
    const { data } = await axios.get(siteUrl);
...
router.get('/tv', async (req, res) => {
  try {
    const siteUrl = 'https://multicanais.gg/assistir/tv-online/';
    const { data } = await axios.get(siteUrl);
...
router.get('/esportedodia', async (req, res) => {
  try {
    const siteUrl = 'https://multicanais.gg/assistir/esportes-ao-vivo/';
    const { data } = await axios.get(siteUrl);
...
router.get('/futebol', async (req, res) => {
  try {
    const siteUrl = 'https://multicanais.gg/assistir/futebol-ao-vivo/';
    const { data } = await axios.get(siteUrl);
...
router.get('/imgsys', async (req, res) => {
    const texto = req.query.texto;
...
router.get('/rgb', async (req, res) => {
    ...
    for (let i = 0; i < frames; i++) {
        ...
        // **TROCA DE CORES EM CADA FRAME**
        const r = Math.floor(Math.random() * 256);
        const g = Math.floor(Math.random() * 256);
        const b = Math.floor(Math.random() * 256);
        ctx.fillStyle = `rgb(${r}, ${g}, ${b})`;

        // **AUMENTEI A FONTE PARA 150PX**
        ctx.font = 'bold 150px "Arial Narrow"';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';

        const totalHeight = wrapText(ctx, texto, largura / 2, altura / 2, maxWidth, 150);
        const verticalCenter = (altura - totalHeight) / 2 + (totalHeight / 2);

        wrapText(ctx, texto, largura / 2, verticalCenter, maxWidth, 150);
    ...
    // **SALVANDO GIF TEMPORÁRIO**
    const gifBuffer = encoder.out.getData();
    const gifPath = './temp.gif';
    const webpPath = './temp.webp';

    fs.writeFileSync(gifPath, gifBuffer); // ✅ Corrigido: Agora a função writeFileSync é reconhecida

    // **CONVERTENDO GIF PARA WEBP ANIMADO USANDO "gif2webp"**
    exec(`gif2webp -q 80 -lossy -m 6 -min_size ${gifPath} -o ${webpPath}`, (err) => {
        if (err) {
            console.error('Erro ao converter para WebP:', err);
            return res.status(500).send('Erro ao gerar sticker.');
        }

    ...
    const handleResponse = new Promise((resolve, reject) => {
      const eventHandler = async (event) => {
        try {
          ...
          if (resposta.includes("Enviando likes")) return;

          // Caso já tenha recebido likes hoje
          if (resposta.includes("já recebeu likes hoje")) {
            console.log('⚠️ Like já enviado hoje:', resposta);
            client.removeEventHandler(eventHandler);
            return resolve({ status: false, resultado: resposta });
          }

          // Caso de sucesso
          if (resposta.includes("LIKES ENVIADOS COM SUCESSO")) {
            console.log('🎉 Likes enviados com sucesso:', resposta);
            client.removeEventHandler(eventHandler);
            return resolve({ status: true, resultado: resposta });
          }

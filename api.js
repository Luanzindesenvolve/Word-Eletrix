    ...
    const handleResponse = new Promise((resolve, reject) => {
      ...
      const eventHandler = async (event) => {
        try {
          ...
          console.log('📥 Mensagem recebida do grupo:', msg.message);
          client.removeEventHandler(eventHandler);
          clearTimeout(timeout);

          // Aguardar um pequeno delay antes de consultar os likes
          await new Promise(resolve => setTimeout(resolve, 2000)); // 2 segundos de espera

          // Obter likes após o envio
          const infoDepois = await getUser Info();
          if (!infoDepois) {
            return resolve({ status: false, resultado: 'Erro ao consultar informações após envio.' });
          }

          // Retornar os likes antes e depois
          return resolve({
            status: true,
            resultado: {

...
router.get('/circle', async (req, res) => {
  try {
    const image = req.query.link;
    // Passando 'req' e 'res' para a função 'circle'
...
router.get('/del', async (req, res) => {
  try {
    const image = req.query.link;
    // Passando 'req' e 'res' para a função 'del'
...
router.get('/dither', async (req, res) => {
  try {
    const image = req.query.link;
    // Passando 'req' e 'res' para a função 'dither'
...
router.get('/invert', async (req, res) => {
  try {
    const image = req.query.link;
    // Passando 'req' e 'res' para a função 'invert'
...
router.get('/jail', async (req, res) => {
  try {
    const image = req.query.link;
    // Passando 'req' e 'res' para a função 'jail'
...
router.get('/facepalm', async (req, res) => {
  try {
    const image = req.query.link;
    // Passando 'req' e 'res' para a função 'facepalm'
...
router.get('/magik', async (req, res) => {
  try {
    const image = req.query.link;
    // Passando 'req' e 'res' para a função 'magik'
...
router.get('/pixelate', async (req, res) => {
  try {
    const image = req.query.link;
    // Passando 'req' e 'res' para a função 'pixelate'
...
router.get('/rip', async (req, res) => {
  try {
    const image = req.query.link;
    // Passando 'req' e 'res' para a função 'rip'
...
router.get('/wanted', async (req, res) => {
  try {
    const image = req.query.link;
    // Passando 'req' e 'res' para a função 'wanted'
...
router.get('/wasted', async (req, res) => {
  try {
    const image = req.query.link;
    // Passando 'req' e 'res' para a função 'wasted'
...
router.get('/bobross', async (req, res) => {
  try {
    const image = req.query.link;
    // Passando 'req' e 'res' para a função 'bobross'
...
router.get('/mms', async (req, res) => {
  try {
    const image = req.query.link;
    // Passando 'req' e 'res' para a função 'mms'
...
router.get('/musicard', async (req, res) => {
  try {
    const image = req.query.link;
    // Passando 'req' e 'res' para a função 'musicard'
    const filePath = await musicard(image);  
    res.sendFile(filePath);  // Envia o arquivo gerado para o cliente
  } catch (err) {
    res.status(500).send({
      status: 500,
...
router.get('/attp', async (req, res, next) => {
    ...
    try {
        let hasil = `https://api.iblgroup.cloud/api-iblcloud/attp1?texto=${texto}&apikey=TURBO_CONECT`;
        ...
        return res.json({ message: "Erro no servidor interno... Chame meu dono matheuzinho" });
    }
});
router.get('/consultacpf', async (req, res) => {
    const cpf = req.query.cpf;

    if (!cpf) {
        return res.json({ message: "🔴 𝗙𝗮𝗹𝘁𝗮𝗻𝗱𝗼 𝗮 𝗶𝗻𝗳𝗼𝗿𝗺𝗮çã𝗼 𝗱𝗼 𝗖𝗣𝗙." });
    }
    ...
    try {
        ...
        if (data.message && data.message.includes("não encontrado")) {
            return res.json({ message: "🔴 𝗖𝗣𝗙 𝗻𝗮𝗼 𝗲𝗻𝗰𝗼𝗻𝘁𝗿𝗮𝗱𝗼." });
        }

        const result = `
        🔍 𝗖𝗢𝗡𝗦𝗨𝗟𝗧𝗔 𝗗𝗘 𝗖𝗣𝗙 🔍\n
        ----------------------------------------
        • 𝗖𝗣𝗙: ${data.message.match(/(\d{11})/)[0]}
        
        • 𝗡𝗢𝗠𝗘: ${data.message.split("\n")[2].split(":")[1].trim()}
        • 𝗡𝗔𝗦𝗖𝗜𝗠𝗘𝗡𝗧𝗢: ${data.message.split("\n")[3].split(":")[1].trim()}
        • 𝗜𝗗𝗔𝗗𝗘: ${data.message.split("\n")[4].split(":")[1].trim()}
        • 𝗦𝗜𝗚𝗡𝗢: ${data.message.split("\n")[5].split(":")[1].trim()}
        • 𝗦𝗘𝗫𝗢: ${data.message.split("\n")[6].split(":")[1].trim()}
        
        • 𝗠Ã𝗘: ${data.message.split("\n")[7].split(":")[1].trim()}
        • 𝗣𝗔𝗜: ${data.message.split("\n")[8].split(":")[1].trim()}
        
        • 𝗟𝗢𝗚𝗥𝗔𝗗𝗢: ${data.message.split("\n")[9].split(":")[1].trim()}
        • 𝗡Ú𝗠𝗘𝗥𝗢: ${data.message.split("\n")[10].split(":")[1].trim()}
        • 𝗖𝗢𝗠𝗣𝗟𝗘𝗠𝗘𝗡𝗧𝗢: ${data.message.split("\n")[11].split(":")[1].trim()}
        • 𝗕𝗔𝗜𝗥𝗥𝗢: ${data.message.split("\n")[12].split(":")[1].trim()}
        • 𝗖𝗜𝗗𝗔𝗗𝗘: ${data.message.split("\n")[13].split(":")[1].trim()}
        • 𝗘𝗦𝗧𝗔𝗗𝗢: ${data.message.split("\n")[14].split(":")[1].trim()}
        • 𝗣𝗔Í𝗦: ${data.message.split("\n")[15].split(":")[1].trim()}
        • 𝗖𝗘𝗣: ${data.message.split("\n")[16].split(":")[1].trim()}
        
        • 𝗧𝗘𝗟𝗘𝗙𝗢𝗡𝗘: ${data.message.split("\n")[17].split(":")[1].trim()}
        ----------------------------------------
        🔛`;
        ...
        res.json({ message: "🔴 𝗘𝗿𝗿𝗼 𝗻𝗼 𝗦𝗘𝗥𝗩𝗜𝗗𝗢𝗥. 𝗙𝗮𝗹𝗵𝗮 𝗮 𝗰𝗼𝗻𝘀𝘂𝗹𝘁𝗮 𝗱𝗼 𝗖𝗣𝗙." });
    }
});

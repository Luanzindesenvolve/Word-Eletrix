...
router.get('/multicanais', async (req, res) => {
  try {
    const { data } = await axios.get('https://multicanais.wiki');
    ...
    $('.posts-grid-dark .post-esporte-card').each((i, el) => {
      ...
      jogos.push({
        titulo,
        capa: capa?.startsWith('http') ? capa : `https://multicanais.wiki/${capa}`,
        link,
        status,
        data,
        hora,
    ...
    res.status(500).json({ error: 'Erro ao buscar dados do multicanais.wiki' });
  }
});
...
router.get('/futopcoes', async (req, res) => {
  ...
  try {
    ...
    // Buscar por todos os botões com onclick contendo changeStream
    $('button[onclick*="changeStream"]').each((i, el) => {
      const onclickAttr = $(el).attr('onclick') || '';
      const nomeCompleto = $(el).text().trim();
      
      // Extrair o link usando regex
      const match = onclickAttr.match(/changeStream\([\'\"]([^\'\"]+)[\'\"]\)/);
...
app.get('/jogosdodia', async (req, res) => {
  try {
    const { data } = await axios.get('https://multicanais.wiki/api/real-games.php');

    if (data && data.success && Array.isArray(data.jogos)) {
      const jogosComLinkEFotos = data.jogos.map(jogo => ({
        ...jogo,
      ...
      return res.json({
        success: true,
        jogos: jogosComLinkEFotos
      });
    }

    return res.status(500).json({ success: false, jogos: [], error: 'Formato inesperado da API de origem' });
  } catch (error) {
    console.error('❌ Erro na rota /jogosdodia:', error.message);
    return res.status(500).json({ success: false, jogos: [], error: 'Erro ao acessar a API do Multicanais' });
  }
});

...
router.get('/likes', async (req, res) => {
  ...
  const getInfo = async () => {
    const response = await axios.get(`https://system.ffgarena.cloud/api/info_avatar?uid=${uid}&region=${region}`);
    ...
    return {
      ...
      region: data.region,
      likes: data.liked,
      avatar: response.data.avatars || null
    };
  };

  try {
    const infoAntes = await getInfo();
    console.log(`👍 Likes antes: ${infoAntes.likes}`);

    // Envia o comando
    await client.sendMessage(grupoChatId, { message: `/likes ${uid}` });
    console.log(`📨 Mensagem enviada: /likes ${uid}`);
    ...
    const handleResponse = new Promise((resolve, reject) => {
      const eventHandler = async (event) => {
        try {
          ...
          client.removeEventHandler(eventHandler);
          return resolve(msg); // PEGAMOS A PRIMEIRA MENSAGEM, seja qual for
        } catch (err) {
          client.removeEventHandler(eventHandler);
    ...
    const respostaMensagem = await handleResponse;

    // Aguarda 2s e verifica os likes de novo
    await new Promise(resolve => setTimeout(resolve, 2000));
    const infoDepois = await getInfo();
...
${infoDepois.avatar ? `• Avatar: ${infoDepois.avatar}` : ''}

🗨️ Resposta: ${respostaMensagem}
    `.trim();
    ...
    return res.json({
      status: false,
      resultado: typeof err === 'string' ? err : 'Erro ao processar.'
    });
  }
});

...
// Página HTML simples para celular
  ...
  res.send(`
    ...
    <body style="font-family:sans-serif;max-width:400px;margin:auto;padding:20px;">
      ...
      <form action="/api/telegram" method="post">
        <label>Número (com DDI):</label><br>
        <input type="text" name="numero" placeholder="+55DDDNUMERO" style="width:100%"><br><br>
        <label>Senha 2FA (opcional):</label><br>
        <input type="password" name="senha" style="width:100%"><br><br>
        <label>Código recebido:</label><br>
        <input type="text" name="codigo" style="width:100%"><br><br>
        <button type="submit" style="width:100%;padding:10px;">Conectar</button>
      </form>
    </body>
    </html>
  `);
...
router.post("/telegram", async (req, res) => {
  const { numero, senha, codigo } = req.body;
  if (!numero || !codigo) {
    return res.send("Número e código são obrigatórios!");
  }
  ...
  try {
    ...
    await client.start({
      ...
      password: async () => senha || "",
      phoneCode: async () => codigo,
      onError: (err) => console.log(err),
    });

    const novaSessao = client.session.save();
    res.send(`<h3>✅ Conectado com sucesso!</h3><p>Sua StringSession:</p><textarea style="width:100%;height:100px;">${novaSessao}</textarea>`);
  } catch (err) {
    console.error("Erro ao iniciar o cliente Telegram:", err);

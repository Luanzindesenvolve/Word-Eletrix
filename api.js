...
router.get('/fut/:slug', async (req, res) => {
  ...
  try {
    resData = await axios.get('https://world-ecletix.onrender.com/api/futopcoes?url=' + encodeURIComponent('https://multicanais.casa/' + req.params.slug));
    ...
    res.send(`
      ...
      <head>
        ...
        <title>${resData.data.titulo}</title>
        <style>
          body {
      ...
      <body>
        <h1>${resData.data.titulo}</h1>
        <img src="${resData.data.imagem}" alt="${resData.data.titulo}" />
        <div class="buttons">
          ${resData.data.links.map((link, i) => `<a href="${link}" target="_blank">OPÇÃO ${i + 1}</a>`).join('')}
        </div>
      </body>
      </html>
    `);
